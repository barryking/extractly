...
// ─── 8. PDF with fragmented form-style text objects ───
// Reproduces form PDFs where words are split across separate text objects and
// tiny whitespace artifacts appear between them. Without artifact filtering and
// stricter same-line spacing, this can produce false line breaks and spaces.

function generateFragmentedForm(): void {
  const builder = new PdfBuilder();
  ...
  const stream = [
    // Artifact-driven heading splits
    'BT', '/F1 12 Tf', '1 0 0 1 72 740 Tm', '(O) Tj', 'ET',
    'BT', '/F1 1 Tf', '1 0 0 1 78 743 Tm', '( ) Tj', 'ET',
    'BT', '/F1 12 Tf', '1 0 0 1 79 740 Tm', '(rder Form) Tj', 'ET',

    'BT', '/F1 12 Tf', '1 0 0 1 72 720 Tm', '(Order) Tj', 'ET',
    'BT', '/F1 1 Tf', '1 0 0 1 104 723 Tm', '( ) Tj', 'ET',
    'BT', '/F1 12 Tf', '1 0 0 1 110 720 Tm', '(Term) Tj', 'ET',

    'BT', '/F1 12 Tf', '1 0 0 1 72 700 Tm', '(Subs) Tj', 'ET',
    'BT', '/F1 1 Tf', '1 0 0 1 100 703 Tm', '( ) Tj', 'ET',
    'BT', '/F1 12 Tf', '1 0 0 1 101 700 Tm', '(cription Details) Tj', 'ET',

    'BT', '/F1 12 Tf', '1 0 0 1 72 680 Tm', '(Billing De) Tj', 'ET',
    'BT', '/F1 1 Tf', '1 0 0 1 124 683 Tm', '( ) Tj', 'ET',
    'BT', '/F1 12 Tf', '1 0 0 1 125 680 Tm', '(tails) Tj', 'ET',

    // Tight same-line joins that should not become internal word spaces
    'BT', '/F1 12 Tf', '1 0 0 1 72 650 Tm', '(The final date is listed) Tj', 'ET',
    'BT', '/F1 12 Tf', '1 0 0 1 72 630 Tm', '(lis) Tj', 'ET',
    'BT', '/F1 12 Tf', '1 0 0 1 87.5 630 Tm', '(ted below) Tj', 'ET',

    'BT', '/F1 12 Tf', '1 0 0 1 72 610 Tm', '(This offer stays valid until the offe) Tj', 'ET',
    'BT', '/F1 12 Tf', '1 0 0 1 249 610 Tm', '(r expiration date above) Tj', 'ET',

    'BT', '/F1 12 Tf', '1 0 0 1 72 590 Tm', '(Seats are added to the Order Term\\) when applicable) Tj', 'ET',

    'BT', '/F1 12 Tf', '1 0 0 1 72 570 Tm', '(Figma then-current list price applies) Tj', 'ET',
  ].join('\n');

  builder.addObject(4, `<< /Length ${stream.length} >>\nstream\n${stream}\nendstream`);
  builder.addObject(5, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>');

  save('fragmented-form.pdf', builder.build());
}

// ─── 9. PDF with bold and italic fonts ───
...
// ─── 12. PDF with flipped Y-axis (CTM with [1, 0, 0, -1, 0, 792]) ───
...
// ─── 13. PDF with cross-reference stream (instead of traditional xref table) ───
...
// ─── 14. PDF with DocuSign-style form anchor tags ───
